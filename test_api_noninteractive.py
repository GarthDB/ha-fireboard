#!/usr/bin/env python3
"""
FireBoard API Connection Test Script (Non-Interactive)

Usage:
    export FIREBOARD_EMAIL="your@email.com"
    export FIREBOARD_PASSWORD="yourpassword"
    python3 test_api_noninteractive.py

Or:
    python3 test_api_noninteractive.py your@email.com yourpassword
"""
...
async def test_fireboard_api(email, password):
    ...
    # Correct API endpoint from FireBoard documentation
    api_base_url = "https://fireboard.io/api"
    
    async with aiohttp.ClientSession() as session:
        # Test 1: Authentication
        print_header("Test 1: Authentication")
        try:
            # FireBoard uses rest-auth/login/ endpoint and expects "username" field
            async with session.post(
                f"{api_base_url}/rest-auth/login/",
                ...
                if response.status == 401:
                    ...
                    print_error("Authentication endpoint not found - this shouldn't happen")
                ...
                # Extract token - FireBoard uses "key" field for the token
                token = auth_data.get("key") or auth_data.get("auth_token") or auth_data.get("token")
                if not token:
        ...
        try:
            # Try v1 API without .json first
            async with session.get(
                f"{api_base_url}/v1/devices",
                headers=headers,
                timeout=aiohttp.ClientTimeout(total=10)
            ) as response:
                if response.status == 404 or response.status == 405:
                    print_warning(f"Endpoint returned {response.status}, trying with .json extension")
                    async with session.get(
                        f"{api_base_url}/v1/devices.json",
                        headers=headers,
                        timeout=aiohttp.ClientTimeout(total=10)
                    ) as retry_response:
                        if retry_response.status == 200:
                            devices = await retry_response.json()
                            print_success("Retrieved devices with .json extension")
                        else:
                            print_error(f"Alternate endpoint also failed: {retry_response.status}")
                            text = await retry_response.text()
                            print_warning(f"Response (truncated): {text[:500]}")
                            
                            # Try one more endpoint - maybe it's under user or account
                            print_warning("Trying user/me or account endpoint...")
                            async with session.get(
                                f"{api_base_url}/rest-auth/user/",
                                headers=headers,
                                timeout=aiohttp.ClientTimeout(total=10)
                            ) as user_response:
                                if user_response.status == 200:
                                    ...
                                    return False  # Can't proceed without devices
                                else:
                                    print_error("Could not find devices endpoint")
                                    return False
        ...
        try:
            # Try v1 API endpoint
            async with session.get(
                f"{api_base_url}/v1/devices/{device_uuid}.json",
                headers=headers,
                timeout=aiohttp.ClientTimeout(total=10)
            ) as response:
                if response.status == 404:
                    print_warning("Trying alternate temperature endpoint")
                    async with session.get(
                        f"{api_base_url}/v1/devices/{device_uuid}",
                        headers=headers,
                        timeout=aiohttp.ClientTimeout(total=10)
                    ) as alt_response:
...
async def main():
    ...
    if len(sys.argv) >= 3:
        ...
        email = os.getenv("FIREBOARD_EMAIL")
        password = os.getenv("FIREBOARD_PASSWORD")
    
    if not email or not password:
        print_error("Missing credentials!")
        print_info("\nUsage Option 1 - Command line:")
        print_info("  python3 test_api_noninteractive.py your@email.com yourpassword")
        print_info("\nUsage Option 2 - Environment variables:")
        print_info("  export FIREBOARD_EMAIL='your@email.com'")
        print_info("  export FIREBOARD_PASSWORD='yourpassword'")
        print_info("  python3 test_api_noninteractive.py")
        print_info("\nUsage Option 3 - .env file (recommended):")
        print_info("  1. Copy .env.example to .env")
        print_info("  2. Edit .env with your credentials")
        print_info("  3. Run: python3 test_api_noninteractive.py")
        sys.exit(1)
    
    try:
        success = await test_fireboard_api(email, password)
